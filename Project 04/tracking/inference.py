    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                temp[newPos] = temp[newPos]+self.beliefs[oldPos]*prob;
        self.beliefs =  temp
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        for particle_position in self.legalPositions:
            dist = util.manhattanDistance(particle_position, pacmanPosition)
            prob = emissionModel[dist]
            self.belief[particle_position] = self.belief[particle_position]*prob
        ...
        if noisyDistance is None:
            ...
            if self.belief.totalCount()==0:
                ...
                self.particles = list()
                for i in range(self.numParticles):
                    self.particles.append(util.sample(self.belief))
        "*** YOUR CODE HERE ***"
        #util.raiseNotDefined()
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #print str(oldPos)+" - "+str(newPosDist)
            for dist in newPosDist.items():
                temp[dist[0]] = temp[dist[0]] + dist[1]*self.belief[oldPos]
        self.belief = temp
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        possible_joint_locations = list(product(self.legalPositions,self.legalPositions))
        loc_len = len(possible_joint_locations)
        i = 0
        self.particles = []
        for i in range(self.numParticles):
            self.particles.append(possible_joint_locations[i%loc_len])
            i+=1
        """*** YOUR CODE HERE ***"""
    ...
    def observeState(self, gameState):
        ...
        for particle_position in self.particles:
            dist1 = util.manhattanDistance(particle_position[0], pacmanPosition)
            dist2 = util.manhattanDistance(particle_position[1], pacmanPosition)
            prob1 = emissionModels[0][dist1]
            prob2 = emissionModels[1][dist2]
            prob = prob1*prob2
            if noisyDistances[0] is None and noisyDistances[1] is not None:
                particle_position = self.getParticleWithGhostInJail(particle_position,0)
                prob = prob2;
            elif noisyDistances[0] is not None and noisyDistances[1] is None:
                particle_position = self.getParticleWithGhostInJail(particle_position,1)
                prob = prob1
            elif noisyDistances[0] is None and noisyDistances[1] is None:
                particle_position = self.getParticleWithGhostInJail(particle_position, 0)
                particle_position = self.getParticleWithGhostInJail(particle_position, 1)
                prob = 1
            belief[particle_position] += prob
        belief.normalize()
